...
def main():
    ...
    r.sendlineafter('> ', '3')
    r.sendlineafter('plain = ', '00 cc')
    r.recvuntil('cipher = ')
    cipher = bytes.fromhex(r.recvline().strip(b'\n').decode())
    #  cipher = [ int.from_bytes(cipher[ i:i+16 ], 'little') for i in range(0, len(cipher), 16) ]
    ...
    flag_cipher = int.from_bytes(flag_cipher, 'little')

#      t0 = bytes_to_long(cipher[:16]) - 0
#      t1 = bytes_to_long(cipher[16:32]) - 0xcc
